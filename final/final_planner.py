...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    delta = [[-1, 1 ], # shift left
             [ 0, 1],  # go straight
             [ 1, 1 ]] # shift right

    delta_name = ['^', '>', 'v']

    delta_cost = [lane_change_cost, 0.0, lane_change_cost]

    value = [[10.0**9 for ss in range(len(road[0]))] for ll in range(len(road))]
    policy = [[((road[ll][ss] == 0) and 'X' or ' ') for ss in range(len(road[0]))] for ll in range(len(road))]
    path = [[((road[ll][ss] == 0) and 'X' or ' ') for ss in range(len(road[0]))] for ll in range(len(road))]

    #Compute the ideal policy
    ...
    while change:
        ...
        for lane in range(len(road)):
            for segm in range(len(road[0])):

                if lane == goal[0] and segm == goal[1]:
                    if value[lane][segm] > 10**-6:
                        change = True
                        value[lane][segm] = 0.0
                        policy[lane][segm] = '*'
                elif road[lane][segm] != 0:
                    for d, sd, cd in zip(delta, delta_name, delta_cost):
                        lane2 = lane + d[0]
                        segm2 = segm + d[1]
                        if lane2 >= 0 and lane2 < len(road) and segm2 >=0 and segm2 < len(road[0]):
                            value2 = value[lane2][segm2] + cd + 1.0 / road[lane][segm]

                            if value2 + 10**-6 < value[lane][segm]:
                                change = True
                                value[lane][segm] = value2
                                policy[lane][segm] = sd

    #for rr in policy:
    #    print rr

    #Apply the ideal policy from the start

    lane = init[0]
    segm = init[1]
    cost = 0.0

    while lane != goal[0] or segm != goal[1]:
        pol = policy[lane][segm]
        di = delta_name.index(pol)
        
        cost += (1.0 / road[lane][segm]) + delta_cost[di]
        lane = lane + delta[di][0]
        segm = segm + delta[di][1]
