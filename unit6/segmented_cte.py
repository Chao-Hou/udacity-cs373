...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        # ----------------------------------------
        # complete path and measurement list

        Z = myrobot.sense()
        filter.sense(Z)

        # ----------------------------------------
        # add points to path and measurement list

        rpath.append([myrobot.x, myrobot.y])
        measurements.append(estimate[:2])
    ...
    return [myrobot.check_goal(goal), myrobot.num_collisions, myrobot.num_steps, rpath, measurements]
...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
    ...
    #Added limit to return values as run also returns the path and measurements
    return run(grid, goal, path.spath, [p_gain, d_gain])[:3]
