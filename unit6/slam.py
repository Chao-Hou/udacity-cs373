...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #Xi has width=2 because of (x,y) coordinates to store

    m_size = N+num_landmarks
    Omega = matrix([[]])
    Xi = matrix([[]])
    Omega = Omega.expand(m_size, m_size, [], [])
    Xi = Xi.expand(m_size, 2, [], [])

    #Insert Initial constraint in Omega and Xi
    #Robot is at center of world and there is no noise

    Omega.value[0][0] = 1.0
    Xi.value[0] = [50.0, 50.0]

    for step, [Z, move] in enumerate(data):
        #Add each motion to the Omega matrix

        Omega.value[step][step] += 1.0 / motion_noise
        Omega.value[step][step+1] -= 1.0 / motion_noise
        Omega.value[step+1][step] -= 1.0 / motion_noise
        Omega.value[step+1][step+1] += 1.0 / motion_noise

        #Add each motion to the Xi matrix

        Xi.value[step][0] -= move[0] / motion_noise
        Xi.value[step][1] -= move[1] / motion_noise
        Xi.value[step+1][0] += move[0] / motion_noise
        Xi.value[step+1][1] += move[1] / motion_noise

        #For each observed landmark at the current step, update the correlations

        for lnum, lx, ly in Z:
            li = N + lnum

            Omega.value[step][step] += 1.0 / measurement_noise
            Omega.value[step][li] -= 1.0 / measurement_noise
            Omega.value[li][step] -= 1.0 / measurement_noise
            Omega.value[li][li] += 1.0 / measurement_noise

            Xi.value[step][0] -= lx / measurement_noise
            Xi.value[step][1] -= ly / measurement_noise
            Xi.value[li][0] += lx / measurement_noise
            Xi.value[li][1] += ly / measurement_noise
            
    MuMatrix = Omega.inverse() * Xi

    #For some reason, all values must be in a single column
    mu = []
    for pos in MuMatrix.value:
        mu.append([pos[0]])
        mu.append([pos[1]])
